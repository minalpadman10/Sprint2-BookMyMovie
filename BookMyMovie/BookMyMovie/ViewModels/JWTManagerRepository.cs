...
namespace BookMyMovie.ViewModels
{
    public class JWTManagerRepository : IJWTMangerRepository
    {
        ...
        public JWTManagerRepository(IConfiguration _configuration, movieDBContext _db)
        {
            db = _db;
            configuration = _configuration;
        }
        public Tokens Authenicate(loginViewModels registerViewModel, bool IsRegister)
        {
            var _isAdmin = false;
            if (IsRegister)
            {
